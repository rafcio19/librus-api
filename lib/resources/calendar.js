  ...
  getTimetable(from, to) {
    ...
    let parser = ($, row) => {
      ...
      lessonCells.each((index, cell) => {
        ...
        // Skip empty cells
        if (!cellText || cellText === '&nbsp;' || cellText.length < 2) {
          list.push(null);
          return;
        }
        ...
        // Check for free days (containing "dzień wolny")
        if (cellText.includes("dzień wolny")) {
          isFreeDay = true;
          status = "free_day";
        }

        // Check for cancelled lessons (wrapped in <s> tags or containing "odwołane")
        else if ($cell.find("s").length > 0 || cellText.includes("odwołane")) {
          isCancelled = true;
          status = "cancelled";
        }

        // Check for substitutions (containing "zastępstwo")
        else if (cellText.includes("zastępstwo")) {
          isSubstitution = true;
          status = "substitution";
        }

        // Find the text div with lesson content
        let textDiv = $cell.find(".text");
        if (textDiv.length === 0) {
          // If no .text div, try to extract from the cell content directly
          textDiv = $cell;
        }
